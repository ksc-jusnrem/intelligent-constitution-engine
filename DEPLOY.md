...
3. Custom domains → add `jusnrem.digital` (and `www.jusnrem.digital`).
...
2. Site settings → Domain management → add custom domain `jusnrem.digital`.
...
2. Add `jusnrem.digital` as the custom domain (creates a `CNAME` file).
...
server {
  ...
  server_name jusnrem.digital www.jusnrem.digital;
